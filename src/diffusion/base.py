...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        if self.loss_type in [
            ...
            with autocast('cuda', enabled=self.conf.fp16, dtype=th.bfloat16 if self.conf.mixed_precision == 'bf16-mixed' else th.float16):
    ...
    def sample(self,
        ...
        if self.conf.model_type.has_autoenc():
            if 'x_start' not in model_kwargs:
                model_kwargs['x_start'] = x_start
            if 'cond' not in model_kwargs:
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                # keep img for next round, before inverting back to pixel space
                img = out["sample"]
