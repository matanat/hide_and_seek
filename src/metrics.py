    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc='lpips'):
            ...
            if use_inverted_noise:
                ...
                x_T = torch.randn((len(imgs), conf.in_channels, conf.img_size, conf.img_size), device=device) if conf.dims == 2 else torch.randn((len(imgs), conf.in_channels, conf.img_size, conf.img_size, conf.img_size), device=device)
    ...
    with torch.no_grad():
        if conf.model_type.can_sample():
            ...
            for i in trange(0, eval_num_images, batch_size, desc=desc):
                ...
                x_T = torch.randn((batch_size, conf.in_channels, conf.img_size, conf.img_size), device=device) if conf.dims == 2 else torch.randn((batch_size, conf.in_channels, conf.img_size, conf.img_size, conf.img_size), device=device)
